...
Por topic: agrupe volume e TFC (top 10 por volume — ordenar por TFC, não por BQS)
```

> `approved` é booleano no Databricks (`true` / `false` / `null`). Não tratar como numérico.
> TFC é a métrica que melhor reflete se o bot conduziu bem a conversa, independente de como o caso terminou. BQS alto com TFC alto indica que o avaliador está aprovando conversas com falha documentada (ex: caso encerrado via transbordo humano → approved=true mesmo com loop ou má interpretação registrada).
